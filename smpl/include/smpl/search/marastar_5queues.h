...
private:
    ...
    struct SearchState : public multi_index_heap_element
    {
        ...
        unsigned int h[5];     // estimated cost-to-go
        unsigned int f[5];     // (g + eps * h) at time of insertion into OPEN
        unsigned int eg;    // g-value at time of expansion
        unsigned short iteration_closed[5];
    ...
    void expand(SearchState* s, sbpl::motion::GroupType group, sbpl::motion::BaseGroupHeuristic base_heur);

    void recomputeHeuristics();
    void reorderOpen();
    int computeKey(SearchState* s, sbpl::motion::GroupType group, sbpl::motion::BaseGroupHeuristic base_heur) const;

    SearchState* getSearchState(int state_id);
    SearchState* createState(int state_id);
    void reinitSearchState(SearchState* state);
    void reinitSearchState(SearchState* state, sbpl::motion::GroupType planning_group, sbpl::motion::BaseGroupHeuristic base_heur);
    ...
    int runSubGroup(SearchState* goal_state, int& elapsed_expansions, 

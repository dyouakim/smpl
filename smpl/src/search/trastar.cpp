...
int TRAStar::replan(const TimeParameters& params,std::vector<int>* solution,int* cost)
{
    ...
    if (m_goal_state_id != m_last_goal_state_id  || costChanged) 
    {
...
void TRAStar::expand(TRAState* s)
{
    ...
    for (size_t sidx = 0; sidx < succs.size(); ++sidx) 
    {
        ...
        ROS_DEBUG_NAMED(SELOG, "Compare new cost %d vs old cost %d for state %i", new_cost, succ_state->g, s->state_id);
...
bool TRAStar::RestoreSearchTree(unsigned int expansionStep)
{
    ROS_WARN_STREAM("In restore");
    m_open.clear();
    m_states.clear();

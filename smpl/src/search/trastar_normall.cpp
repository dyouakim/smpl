...
void TRAStar::expand(TRAState* s)
{
    ...
    m_space->GetSuccsWithExpansion(s->state_id, &succs, &costs,expansion_step);
    ...
    for (size_t sidx = 0; sidx < succs.size(); ++sidx) 
    {
        ...
        if(succ_state_id==0)
            ROS_WARN_STREAM("after get value is  "<<succ_state->state_id<<" and g-val "<<succ_state->g);
        reinitSearchState(succ_state);
        if(succ_state_id==0)
            ROS_WARN_STREAM("after reinit value is  "<<succ_state->state_id<<" and g-val "<<succ_state->g);
        if(succ_state_id==0)
        {
...
bool TRAStar::RestoreSearchTree(int restoreStep)
{
    ...
    else
    {
        ...
        ROS_INFO_STREAM("in restore seen states are "<<seen_states.size());
        TRAState* parent;
        unsigned int parentGVal;
        for(int i=0;i<seen_states.size();i++)
        {
            TRAState* current = seen_states[i];
            ROS_INFO_STREAM("Seen State "<<current->state_id<< " has  C "<<current->C<< " & has  E "<<current->E);
            //state created and expanded
            if(current->E <= restoreStep)
            {
                ROS_INFO_STREAM("Fully Restored!");
            ...
            {
                ROS_INFO_STREAM("State generated only!");
                updateParents(current,restoreStep,current->bestpredstate,&parentGVal);
                current->g = parentGVal;
                ...
                current_seen.push_back(current);
                m_open.push(current);
                ROS_INFO_STREAM("State generated only with C, E, g, h "<<current->C);
            }
            //state not created yet
            else
            {
                ROS_INFO_STREAM("Clear State!");
                current->v = INFINITECOST;
...
bool TRAStar::updateParents(TRAState* state, unsigned int expansionStep, TRAState* latestParent, unsigned int *latestGVal)
{
    ...
    *latestGVal = 0;
    ...
    for(int i=0;i<state->parent_hist.size();i++)
    {
        ...
        {
            ROS_INFO_STREAM("Erasing Parent with id "<<parent->state_id<<", E "<<parent->E);
